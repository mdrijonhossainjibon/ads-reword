...
export default function UserStats({ userName, watchedAds = 0, earnedPoints = 0 }: UserStatsProps) {
  return (
    <div className="w-full max-w-md bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 mb-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gray-700/50 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-blue-400">{watchedAds}</p>
        </div>
        <div className="bg-gray-700/50 rounded-xl p-4">
          <p className="text-sm text-gray-400 mb-1">Earned Points</p>
          <p className="text-2xl font-bold text-green-400">{earnedPoints.toFixed(2)}</p>
        </div>
      </div>
    </div>
  );
